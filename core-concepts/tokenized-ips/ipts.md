...
IP Tokens (IPTs) solve this by fractionalizing IP-NFT ownership into fungible ERC-20 tokens. When an IP-NFT is tokenized, the Tokenizer records the tokenization and deploys a corresponding IPT — the IP-NFT itself remains held by its owner (typically the Lab's Token Bound Account). Each IPT represents a fractional stake in the underlying intellectual property — granting the holder governance rights over decisions affecting the IP, the ability to participate in fundraising rounds, and a claim on revenue generated by the Lab (such as licensing fees, data access payments, and milestone payouts).
...
In the V3 architecture, tokenization happens inside Onchain Labs. When a Lab tokenizes an IP-NFT, the Lab's Token Bound Account (as the IP-NFT's controller) calls the Tokenizer and receives the initial IPT supply — the IP-NFT stays in the Lab's custody throughout. The IPTs are then held in the Lab's treasury until distributed through fundraising, grants, or direct allocation.
...
3. **Call `tokenizeIpnft()`** — The Tokenizer records the tokenization and deploys a new ERC-20 contract with the specified symbol and initial supply; the IP-NFT remains with its owner
4. **Receive IPT** — The initial supply is minted to the caller — the IP-NFT's controller, i.e. the Lab's Token Bound Account

**Tokenization inputs:**

<table><thead><tr><th width="211.9140625">Parameter</th><th>Description</th></tr></thead><tbody><tr><td>tokenSymbol</td><td>Token symbol (at least 1 character)</td></tr><tr><td>tokenAmount</td><td>Initial token supply</td></tr><tr><td>agreementCid + signedAgreement</td><td>The membership agreement CID and the caller's signature over its terms</td></tr></tbody></table>
...
* **`issue(address receiver, uint256 amount)`** — Mint additional tokens to a specified address, typically for fundraising rounds or contributor allocation
* **`cap()`** — Permanently freeze the supply at its current level (irreversible, no arguments). Once called, no more tokens can ever be minted. This provides supply certainty to token holders and investors
...
// Freeze supply at current level (irreversible)
...
```solidity
// Create IPT from IP-NFT (initial supply mints to the caller)
...
// Attach existing ERC-20 as IPT (wrapped, read-only)
function attachIpt(
    uint256 ipnftId,
    string memory agreementCid,
    bytes calldata signedAgreement,
    IERC20Metadata tokenContract
) external returns (IIPToken)

// Mint additional IPT
function issue(IPToken ipToken, uint256 amount, address receiver) external
