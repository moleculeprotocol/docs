...
Access is verified via the `canRead()` view function (backed by the `readAllowances` mapping).

{% hint style="warning" %}
**Deprecated.** IP-NFT-level read grants are a legacy mechanism: only files encrypted under the legacy (Lit-era) access conditions still evaluate `canRead`. For current files, data-room access is managed at the **Lab** level — the Lab owner (and Contributors, for Viewer grants) grant roles through the `AccessResolver`, and decryption conditions check `hasRole` / `isAuthorizedSignerForTba`. See [Roles & Permissions](../roles-and-permissions.md) and the **Data Privacy & Access** page.
{% endhint %}
