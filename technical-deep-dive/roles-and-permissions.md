...
Five capabilities can be opened, each independently: adding files, modifying them (new versions, metadata, moves), deleting them, creating announcements, and decrypting encrypted files. Each is set to one of three rule kinds — membership only (the default), any authenticated caller, or any caller whose wallet satisfies an [access-control-condition array](data/data-privacy-and-access.md#condition-shape) such as holding a token. A rule can additionally be time-boxed to a deadline, or wired to a caller-independent condition that closes it once it becomes true — "permissionless until this bounty contract reports itself closed". Decrypting is the odd one out: its default is Viewer-or-above rather than Contributor-or-above, and it is never opened automatically by the `OPEN` preset — an open Lab's encrypted files stay member-readable-only until the owner adds an explicit rule.
