...
| Network          | Address                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Ethereum Mainnet | [`0xF0A8D23F38E9CbBe01C4Ed37f23BD519b65BC6C2`](https://etherscan.io/address/0xF0A8D23F38E9CbBe01C4Ed37f23BD519b65BC6C2)         |
| Sepolia Testnet  | [`0x8cA737E2cdaE1Ceb332bEf7ba9eA711a3a2f8037`](https://sepolia.etherscan.io/address/0x8cA737E2cdaE1Ceb332bEf7ba9eA711a3a2f8037) |
...
Extends LockingCrowdSale by requiring bidders to stake a secondary token alongside their bid. The staking token and amount ratio are configured at sale start. Staked tokens are vested for a **separately configured duration** (minimum 7 days), independent of the auction-token locking period, and are returned to the bidder when they claim. If the sale fails, both the bid and the staked tokens are returned.
...
* **Started** — Emitted when a new sale begins. Includes the full Sale struct and fee configuration. The locking and staking variants emit extended versions that additionally include the locking token & duration (LockingCrowdSale) and the staking configuration (StakedLockingCrowdSale).
...
* **FeesUpdated** — Emitted when the owner changes the protocol fee.
...
* **Reentrancy protection**: the `settle()` and `claim()` operations use OpenZeppelin's ReentrancyGuard.
...
* **Source Code**: [CrowdSale.sol](https://github.com/moleculeprotocol/IPNFT/blob/main/src/crowdsale/CrowdSale.sol), [LockingCrowdSale.sol](https://github.com/moleculeprotocol/IPNFT/blob/main/src/crowdsale/LockingCrowdSale.sol), [StakedLockingCrowdSale.sol](https://github.com/moleculeprotocol/IPNFT/blob/main/src/crowdsale/StakedLockingCrowdSale.sol)
* **ABI**: Generated from the contract source in the [IPNFT repository](https://github.com/moleculeprotocol/IPNFT)
* **Audit**: [Pashov security review](https://github.com/pashov/audits/blob/master/solo/pdf/IPNFT-security-review.pdf)
