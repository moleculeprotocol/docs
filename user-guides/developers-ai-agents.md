...
For data room interactions (showing a Lab's files, uploading research data on behalf of a user), use the Labs API. File uploads follow a three-step flow: call `initiateCreateOrUpdateFile` to get a presigned S3 URL, PUT the file to that URL, then call `finishCreateOrUpdateFile` with metadata. If the file needs encryption, first request a key via `generateDataEncryptionKey(oclId, accessControlConditions)` — the backend returns a one-shot plaintext DEK bound to that lab and condition array, plus its encrypted form — and AES-256-GCM encrypt the file locally via Web Crypto before uploading, attaching the same `accessControlConditions` and the encryption metadata on finish. On download, `decryptDataKey` returns the unwrapped DEK after checking the lab's `DECRYPT_FILES` capability and re-verifying the file's own access conditions against live onchain state. The [Data Privacy & Access](../technical-deep-dive/data/data-privacy-and-access.md) page walks through both flows end-to-end.
