...
| `api-reference/getting-started/for-agents.md` | Condensed mirror of Tutorial 1 plus the error contract. Sources: `graphql/schemas/ip-hubs.graphql`, `graphql/schemas/api-error.graphql`, `graphql/schemas/encryption.graphql`, `lambda/appsync-resolver-labs-lambda/**` (per-mutation role gates). **Must stay consistent with `example-workflow.md`** — a change to the flow touches both pages or neither. |
| `api-reference/labs-api/README.md` | `graphql/schemas/ip-hubs.graphql`, `lambda/appsync-resolver-labs-lambda/**` |
| `api-reference/labs-api/example-workflow.md` | The tutorials page (H1 "Tutorials"; the filename is kept for inbound links). The end-to-end flow across four tutorials: `graphql/schemas/ip-hubs.graphql`, `graphql/schemas/encryption.graphql`, `lambda/appsync-resolver-labs-lambda/**` (especially the per-mutation authorization gates — `authorizeServiceAdmin` = Owner, `authorizeServiceMember` = Contributor), `services/token-manager-service.ts`, `lib/utils/token-expiration.ts` (the `expiresIn` bounds). Every code block is expected to be runnable, so a signature change here is a real breakage — correct it and say so in the PR body. |
